...
def get_word_rating(data, model, tokenizer, gold=None):    
    ...
    ig_values = []
    for sent in data:
        ig_values.append(explainer(sent))
        ...
        if args.do_alignment:
            ...
            for index_word, word in enumerate(sent_spacy):
                ...
                if alignment[index_word]:
                    ...
                    for index in alignment[index_word]:
                        try:
                            value += ig_values[index_sent][index][1]
                        except IndexError:
            ...
            for index_word, word in enumerate(sent_bert):
                if word not in tokenizer.special_tokens_map.values():
                    if word not in word2values:
                        word2values[word] = [ig_values[index_sent][index_word]]
                    else:
                        word2values[word].append(ig_values[index_sent][index_word])
    ...
    output_file = os.path.join(args.output_dir, args.dataset+'_'+args.model_kind+'_'+args.task+'_ig.csv')
...
if __name__=="__main__":
    ...
    get_word_rating(df_train.text.values, model, tokenizer, args.gold_word)