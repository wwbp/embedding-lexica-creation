...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, value=None):
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
      guid = "%s-%s" % (set_type, i)
      text_a = tokenization.convert_to_unicode(line[7].strip('\"'))
      if set_type == "test":
        value = "-1"
      else:
        value = tokenization.convert_to_unicode(line[3].strip('\"'))
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=None, value=value))
    return examples
...
def convert_single_example(ex_index, example, max_seq_length,
  ...
  if ex_index < 5:
    ...
    tf.logging.info("value: %s" % (example.value))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["values"] = create_float_feature([feature.value])
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "values": tf.FixedLenFeature([], tf.float32),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    loss = tf.losses.mean_squared_error(values, output)
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if "is_real_example" in features:
      ...
      is_real_example = tf.ones(tf.shape(values), dtype=tf.float32)
    
    is_training = (mode == tf.estimator.ModeKeys.TRAIN)

    (total_loss, pred_values) = create_model(
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(preds, values):

        return {
            "eval_loss": tf.metrics.mean_squared_error(labels=values, predictions=preds, 
                                                       weights=is_real_example),
        }

      eval_metrics = (metric_fn, [values, pred_values])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
    ...
    d = tf.data.Dataset.from_tensor_slices({
        ...
        "segment_ids":
            tf.constant(
                ...
                dtype=tf.int32),
        "values":
            tf.constant(all_values, shape=[num_examples], dtype=tf.float32),
    })
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    train_examples, eval_examples = train_test_split(examples, test_size=0.2)
    num_train_steps = int(
